...
void test_proc() {
    ...
    test_assert(do_waitpid(kill_all_proc->p_pid, &status, 0) == kill_all_proc->p_pid 
        && status == -1, "Kill all process not exit correctly or returned status not set correctly");
